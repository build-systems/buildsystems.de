...
const baseOffset = containerH / 2 - groupCenterInTrack - 15;
...
function render(angle: number) {
  for (let i = 0; i < groups.length; i++) {
    ...
    groups[i].setAttribute(
      "transform",
      `translate(${CX + R * Math.cos(a)},${CY + R * Math.sin(a)})`,
    );

    // Proximity to rightmost position (0°): 1 = at right, 0 = far away
    ...
    circles[i].style.stroke = `rgba(255,255,255,${((1 - proximity) * 0.3).toFixed(3)})`;
